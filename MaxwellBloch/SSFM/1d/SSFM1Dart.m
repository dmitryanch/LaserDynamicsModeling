...
Pf0=1e-4*ones(Nr,1);
Df0=1e-4*ones(Nr,1);
Ef0(1) = sqrt(r-1)/sqrt(2) + 1i*sqrt(r-1)/sqrt(2);
Pf0(1) = Ef(1);
Df0(1)= 1;
Ef0(kfil+1:Nr-kfil+1)=0;
Pf0(kfil+1:Nr-kfil+1)=0;
Df0(kfil+1:Nr-kfil+1)=0;
E0=fft(fftshift(Ef0));
P0=fft(fftshift(Pf0));
D0=fft(fftshift(Df0));
...
K=kx.^2;
%%
Tmax = 20e3;%+10e3;      %общее время счета
...
Puankare=[];
q=0;
multiplierErow=200;       %число обратное тому сколько будет браться точек из Erow
