...
xlabel('t','FontSize',14,'FontWeight','bold');ylabel('I=|E|^2','FontSize',14,'FontWeight','bold');
...
xlabel('t','FontSize',14,'FontWeight','bold');ylabel('\int|E|^2','FontSize',14,'FontWeight','bold');
saveas(gcf,gcffilename,'jpg');close gcf;
%% NEAR FIELD
x0=(0:Nr-1)*h;
Ecur=ifft2(Ef);
figure;
imagesc(x0(Nr*NrecSize+1:Nr*(1-NrecSize)),x0(Nr*NrecSize+1:Nr*(1-NrecSize)),abs(Ecur(Nr*NrecSize+1:Nr*(1-NrecSize),Nr*NrecSize+1:Nr*(1-NrecSize))).^2);
colormap(grayCustom);colorbar('vert');
%% FAR FIELD (STANDART)
figure;
imagesc(kx,kx,abs(fftshift(Ef)));
...
%%
mesh(abs(Ecur).^2);colormap(gray);
xlabel('x','FontSize',14,'FontWeight','bold');ylabel('y','FontSize',14,'FontWeight','bold');     
zlabel('I=|E|^2','FontSize',14,'FontWeight','bold')
saveas(gcf,['mesh ',gcffilename],'png');close gcf;

figure;plot((1:numel(IE))*erow_incr*tau,IE.^2/Nr^4,'black');
xlabel('t','FontSize',14,'FontWeight','bold');
ylabel('\int|E|^2','FontSize',14,'FontWeight','bold');
xlim([1100 1200])
gcffilename=['Integral Int(t) 2D ',filename,'.jpg'];
...
figure;plot((1:numel(INT))*erow_incr*tau,INT,'black');
xlabel('t','FontSize',14,'FontWeight','bold');
ylabel('I=|E|^2','FontSize',14,'FontWeight','bold');
xlim([1100 1200])
gcffilename=['Int(t) 2D ',filename,'.jpg'];
saveas(gcf,['zoom ',gcffilename],'jpg');
xlim([1150 1200])
...
nw=8192;w=(-nw/2:nw/2-1)*2*pi/(tau*nw);plot(w,fftshift(abs(fft(INT(numel(INT)-nw+1:numel(INT))))),'black')
xlim([0 100]);
xlabel('\omega','FontSize',14,'FontWeight','bold');
ylabel('C','FontSize',14,'FontWeight','bold');
