%%%%% возмущения берутся по 4 направлениям сразу
clc
clear
...
y3=[];
q=0:0.4:40;
IMAG=zeros(length([1:0.1:8,8.2:0.2:30]),length(0:0.1:20));q1=0;

for k=0:0.1:20%[0:0.1:6,6.02:0.02:14,14.1:0.1:20] %диапазон значений волнового числа
    q1=q1+1;q2=0;
    for r=[1:0.1:8,8.2:0.2:30]%[(1+(a*k^2-delta)^2/(1+sigma)^2):0.1:8] %диапазон значений накачки
          q2=q2+1;
          Eigs=EigProg1D2D(k,r);
          if all(Eigs<1e-10) %зона устойчивости
              x=[x k];
              y=[y r];              
          elseif all(Eigs(1:5)<1e-10)
              IMAG(q2,q1)=1; %зона коротковолновой неустойчивости
              x1=[x1 k];
              y1=[y1 r];
          elseif all(Eigs(length(q)-87:length(q))<1e-10)
              IMAG(q2,q1)=2; %зона длинноволновой неустойчивости
              x2=[x2 k];
              y2=[y2 r];
